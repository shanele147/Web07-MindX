...
const ExpenseTabs = (props) => {
  const { activeTabIndex, handleTabIndex } = props;
      ...
      return (
        <>
          ...
          >
            <li>
              <p
                className={`category expense-${category
                  .toLowerCase()
  ...
  const expenses = renderDataList(expenseList);
  const incomes = renderDataList(incomeList);

  const getTAmounTotal = (category, list) => {
    return list
      .filter((elm) => elm.category === category)
      .reduce((total, elm) => total + Number(elm.amount), 0);
  };
  const expenseBasedOnCategory = expenseCategories.map(cat => getTAmounTotal(cat, expenseList))
  const incomeBasedOnCategory = incomeCategories.map(cat => getTAmounTotal(cat, incomeList))
      ...
      <div className="flex flex-wrap justify-center">
        ...
        {tabsData.map((tab, idx) => {
          return (
            <div
              key={idx}
              className={`w-full md:w-2/3 lg:w-1/2 py-4 px-4 md:px-6 mx-auto tab-content animate__animated animate__slow ${idx === activeTabIndex ? "animate__fadeIn" : "animate__fadeOut"
                }`}
            >
              {tab.content}
